from utils import EXTENDED_KEY_SCHEDULE_CONST, ROUNDS_NUMBER, PERMUTATION_TABLE, REVERSED_PERMUTATION_TABLE, \
    ROTATION_CONSTANTS, add, sub, rotate_left, rotate_right, bytes_to_words, words_to_bytes
...
class Threefish:
    ...
    def __init__(self, key: bytes, tweak: bytes):
        if len(key) not in [32, 64, 128]:
            raise ValueError(f"Invalid length: The length of the key must be 258, 512 or 1024 bit")

        if len(tweak) != 16:
            raise ValueError(f"Invalid length: The length of the tweak must be 128 bit")
    ...
    def encrypt_block(self, plaintext: bytes):
        ...
        if len(self.k) - 1 != len(plaintext):
            raise ValueError(f"Invalid length: The length of the key must be equal to the length of the plaintext")
    ...
    def decrypt_block(self, ciphertext):
        ...
        if len(self.k) - 1 != len(ciphertext):
            raise ValueError(f"Invalid length: The length of the key must be equal to the length of the ciphertext")
